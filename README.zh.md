...
| 特性 | 说明 |
|------|------|
| 🧩 **Fragment 架构** | 任何语法子树都是合法的顶层 Fragment — `module`、`type`、`flow`、`func`、`ui`、`steps`、表达式、UI 节点 |
...
| 文档 | 说明 |
|------|------|
| [语法规范](docs/specification.md) | 完整语言参考（1329 行） |
...
**Q: `.mms` 和 `.mimi` 有什么区别？**  
A: `.mms`（MimiSpec）是**意图设计层** — 渐进式、人类可读、碎片友好。`.mimi`（Mimi）是**生产编译目标** — 合约验证、LLVM 编译、结构化并发和线性能力。

**Q: 这个可以用于生产吗？**  
A: 解析器（v0.1）已完全可用 — 已发布至 [crates.io](https://crates.io/crates/mimispec)，77 个单元测试通过，支持错误恢复和完整 AST 渲染。CLI 二进制可通过 `cargo install mimispec` 安装。生产工具（跨文件链接、LSP、Mimi 编译）已在路线图。
