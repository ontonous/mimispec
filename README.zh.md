...
> 发布状态：crates.io、Cargo 与最新发布 tag 仍为 `0.2.1`。main 已包含合并版
> 0.3 Core、stdio 语言服务器、规范符合性套件与发布阻断试用门禁。独立试用完成
> 并得到明确发布授权前，发布事实保持不变。
...
> `materialize`、`profile` 与 `workflow` 是 0.3 Core 路线之外的临时研究接口。
...
| 文档 | 说明 |
|------|------|
| [语法规范草案](docs/specification.md) | main 已实现的 0.3 Core 草案；已发布包仍为 0.2.1 |
| [0.3.x 开发路线](docs/roadmap-0.3.x.md) | 从 Core 语义修正、无损解析到稳定化的版本规划 |
| [0.3.x 可用性报告](docs/0.3-usability-report.md) | 真实家庭账本试用、已验证行为与尚未完成的范围 |
...
**Q: 当前发布版本是什么？**
A: 当前发布版本是 `v0.2.1`。解析器、跨文件 resolver、符号表、增量缓存和 CLI 已可用。`0.3.x` 将修正 Context、`desc`、重复条款与 Flow 语义，加入无损文档模型，并把 commitment 协议做实；文档中的前瞻语义不代表已经发布。
