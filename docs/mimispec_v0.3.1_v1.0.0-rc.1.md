...
MimiSpec v0.3.1 以**单文件优先**为设计原则：解析器独立处理每个 `.mms` 文件，不强制解析外部依赖。跨文件引用通过 `@import` 显式声明，但链接工作延迟到 IDE 或编译工具阶段，保持解析层的极简与碎片的独立性。
...
| `$??` | 锁定 | 完全委托 | 我锁定了，但 AI 全权决定是否保留锁 |
| `$$?` | 强锁定 | 不确定 | 我想强锁，但 AI 帮我再审视 |
| `$$??` | 强锁定 | 完全委托 | 我强锁了，但 AI 全权决定是否保留强锁 |
...
> rule$?? "支付必须幂等"    # 用户锁定了，但 AI 可全权决定是否保留锁定
> ```
...
**赋值 (`=`)**：
动作步骤可使用 `=` 对字段或变量赋值。语法：`target = value [to 目标]`。
